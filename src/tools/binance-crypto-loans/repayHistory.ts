// src/tools/binance-crypto-loans/repayHistory.ts
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { z } from "zod";
import { cryptoLoanClient } from "../../config/binanceClient.js";

export function registerBinanceCryptoLoanRepayHistory(server: McpServer) {
    server.tool(
        ...
        async ({ orderId, loanCoin, collateralCoin, startTime, endTime, current, limit }) => {
            try {
                ...
                const response = await cryptoLoanClient.restAPI.repayHistory(params);
                const data = await response.data();
                return {
                    content: [
                        {
                            type: "text",
                            text: `Repay history retrieved. Response: ${JSON.stringify(data)}`
                        }
                    ]
                };
            } catch (error) {
                ...
                return {
                    content: [
                        { type: "text", text: `Failed to get repay history: ${errorMessage}` }
                    ],
                    isError: true
                };
            }
